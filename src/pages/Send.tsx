...
const Send = () => {
  ...
  const calculateDynamicPrice = useCallback((sizeMB: number, pricingData: any) => {
    ...
    // Add reasonable service fee margin (70% as you wanted)
    const totalCostWithMargin = adjustedCostInUSD * 1.7;
    
    // Use normal rounding instead of aggressive rounding
  ...
  useEffect(() => {
    if (files.length > 0) {
      ...
      // Pricing tiers
      if (totalSizeMB < 1) {
        // Below 1MB - $0.05 (for testing only, not shown to users)
        tier = 'Below 1MB';
        fee = '0.05';
        ...
        // For files over 100MB up to 500MB, use dynamic pricing
        ...
        if (arweavePricing && arweavePricing.timestamp > Date.now() - 3600000) { // Cache for 1 hour
          ...
          if (dynamicFee) {
            ...
            // Fallback calculation: base $4 + $0.04 per MB over 100MB
            const extraMB = totalSizeMB - 100;
            fee = (4 + (extraMB * 0.04)).toFixed(2);
          }
        } else {
          // Set initial fee while fetching real-time data
          const extraMB = totalSizeMB - 100;
          fee = (4 + (extraMB * 0.04)).toFixed(2);
  ...
  // Get file type color based on extension and size
  const getFileTypeColor = (fileName: string, fileSize: number) => {
    const extension = fileName.split('.').pop()?.toLowerCase() || '';
    const sizeMB = fileSize / 1024 / 1024;
    
    // Use colors from the second image - dark slate and light gray
    const colorSchemes = {
      dark: {
        light: 'bg-slate-500',    // < 5MB - medium dark
        medium: 'bg-slate-600',   // 5-50MB - darker
        heavy: 'bg-slate-700'     // > 50MB - darkest
      },
      light: {
        light: 'bg-gray-400',     // < 5MB - light gray
        medium: 'bg-gray-500',    // 5-50MB - medium gray
        heavy: 'bg-gray-600'      // > 50MB - darker gray
      }
    };
    
    // Alternate between dark and light colors based on file index or type
    ...
    // Use dark color for media files, light for documents/others
    let colorScheme = 'light';
    if (imageExts.includes(extension) || videoExts.includes(extension)) {
      colorScheme = 'dark';
    }
    
    // Determine size category - bigger files get darker colors
    let sizeCategory = 'light';
    if (sizeMB > 50) sizeCategory = 'heavy';
    else if (sizeMB > 5) sizeCategory = 'medium';
    
    return colorSchemes[colorScheme as keyof typeof colorSchemes][sizeCategory as keyof typeof colorSchemes.dark];
  };
                      ...
                      <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
                        <div className="flex items-center justify-start gap-6">
                          {files.map((file, index) => {
                            ...
                            const colorGradient = getFileTypeColor(file.name, file.size);
          ...
          <div>
            ...
            <div className="glass-panel p-6 mt-6">
              ...
              <ul className="space-y-3 text-sm text-doc-medium-gray">
                ...
                <li className="flex">
                  <span className="text-doc-deep-blue mr-2">•</span>
                  Prices are subject to change according to blockchain network fees
                </li>
              </ul>
            </div>
            ...
            <div className="glass-panel p-6 mt-6">
              ...
              <ul className="space-y-3 text-sm">
                <li className="flex justify-between">
                  <span className="text-doc-medium-gray">1MB - 10MB:</span>
                  <span className="font-medium">$1.00 USDC</span>
                </li>
                <li className="flex justify-between">
                  <span className="text-doc-medium-gray">10MB - 30MB:</span>
                  <span className="font-medium">$2.00 USDC</span>
                </li>
                <li className="flex justify-between">
                  <span className="text-doc-medium-gray">30MB - 100MB:</span>
                  <span className="font-medium">$4.00 USDC</span>
                </li>
                <li className="flex justify-between">
                  <span className="text-doc-medium-gray">Above 100MB:</span>
